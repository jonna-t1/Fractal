...
// }
std::vector<int> mandelbrotFormula(const complex<float> &z0, const int maxIter=500) {
    std::vector<int> arr;
    // int arra [3];
    int value = mandelbrot(z0, maxIter);
    if(maxIter - value == 0)
    ...
    // int x[3] = {cvRound(sqrt(value / (float) maxIter) * 255), 
    // cvRound(sqrt(value / (float) maxIter) * 255), 
    // cvRound(sqrt(value / (float) maxIter) * 255)}
    arr.push_back(cvRound(sqrt(value / (float) maxIter) * 255));
    arr.push_back(cvRound(sqrt(value / (float) maxIter) * 255));
    arr.push_back(cvRound(sqrt(value / (float) maxIter) * 255));
    return arr;
}
...
public:
    ...
    virtual void operator ()(const Range& range) const CV_OVERRIDE
    {
        for (int r = range.start; r < range.end; r++)
        {
            ...
            std::vector<int> arr = mandelbrotFormula(z0);
            int count = 0;
            for (int i : arr){
                m_img.at<cv::Vec3b>(j,i)[count] = (uchar) i;
                count++;
            }

            // Point3_<uchar>* p = m_img.ptr<Point3_<uchar> >(j,i);

            // m_img.ptr<uchar>(i)[j] = value;
            // m_img.at<cv::Vec3b>(j,i)[0] = value; 
...
int main()
{
    ...
    parallel_for_(Range(0, mandelbrotImg.rows*mandelbrotImg.cols), [&](const Range& range){
        for (int r = range.start; r < range.end; r++)
        {
            ...
            for (int i : arr){
                if (count > 2 ) break;
                mandelbrotImg.at<cv::Vec3b>(j,i)[count] = (uchar) i;
                count++;
            }
        }
    });
    ...
    imwrite("../img/Mandelbrot_parallel.png", mandelbrotImg);
